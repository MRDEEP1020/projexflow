...
new #[Layout('layouts.app')] #[Title('Archive')] class extends Component
{
    use WithPagination;
...
<div class="max-w-5xl space-y-5">
    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
        ...
        <flux:button variant="ghost" icon="folder-open" href="{{ route('projects.index') }}" wire:navigate>
            Active projects
        </flux:button>
    </div>
    ...
    @else
        ...
        <flux:table :paginate="$this->projects">
            <flux:table.columns sticky>
                <flux:table.column
                    sortable :sorted="$sortBy === 'name'" :direction="$direction"
                ...
                <flux:table.column
                    sortable :sorted="$sortBy === 'archived_at'" :direction="$direction"
                    wire:click="sort('archived_at')"
                >Archived</flux:table.column>
                                ...
                                <flux:menu>
                                    ...
                                    @if(Auth::user()->is_marketplace_enabled)
                                        <flux:menu.item icon="star"
                                            href="{{ route('portfolio.create', ['project_id' => $project->id]) }}" wire:navigate>
                                            Add to portfolio
                                        </flux:menu.item>
                                    @endif
                                    ...
                                    <flux:menu.item
                                        icon="arrow-path"
                                        wire:click="unarchive({{ $project->id }})"
