                                                ...
                                                @foreach(array_keys($columns) as $s)
                                                    @if($s !== $status)
                                                        <button
                                                            ...
                                                            title="Move to {{ $columns[$s]['label'] }}"
                                                        >{{ $columns[$s]['label'] }}</button>
                                                    @endif
                                                @endforeach
                                            </div>
                                        </div>
            ...
            <div class="w-72 flex-shrink-0 space-y-4 hidden xl:block">
                ...
                <flux:card class="bg-[#0e1420] border-[#1c2e45] !p-4">
                    ...
                    <div class="space-y-2">
                        ...
                        @endforeach
                        @if($members->count() > 6)
                            <p class="text-xs text-[#506070] text-center">+{{ $members->count() - 6 }} more</p>
                        @endif
                    </div>
                </flux:card>
    ...
    <flux:modal wire:model="showQuickAdd" class="max-w-md">
        <div class="space-y-4">
            ...
            <form wire:submit="quickAdd" class="space-y-4">
                <flux:field>
                    <flux:label>Task title <span class="text-red-400">*</span></flux:label>
                ...
                <flux:field>
                    ...
                    <flux:select wire:model="newTaskColumn">
                        @foreach($columns as $s => $col)
                            <flux:select.option value="{{ $s }}">{{ $col['label'] }}</flux:select.option>
                        @endforeach
                    </flux:select>
                </flux:field>
    ...
    @if($openTaskId)
        ...
        >
            ...
            <div class="relative z-50 w-full max-w-xl bg-[#0e1420] border-l border-[#1c2e45] h-full overflow-y-auto shadow-2xl animate-slide-in-right">
                ...
                <div class="p-5">
                    @livewire('backend.taskDetail', ['taskId' => $openTaskId], key('drawer-'.$openTaskId))
                </div>
            </div>
        </div>
    @endif
